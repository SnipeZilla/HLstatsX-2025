...
sub handleIncoming {
    ...
    if (defined $s_output && $s_output ne "") {
        ...
        if ($s_output =~ s/^(?:.*?)?L (\d\d)\/(\d\d)\/(\d{4}) - (\d\d):(\d\d):(\d\d)\.?(\d\d\d)?\s*[:-]\s*//) {
            ...
            &printEvent(998, "MALFORMED DATA: " . $s_output) if $g_debug >= 3;
            return;
        }
...
sub handleData
{
            ...
            if ( $g_servers{$server}->{track_server_timestamp} > 0 ) {
                if ( $g_servers{$server}->{track_server_timestamp} + 299 < $ev_daemontime ) {
                    $g_servers{$server}->track_server_load_async(sub {
                        ...
                        $g_servers{$server}->set("track_server_timestamp", $data->{timestamp});
    ...
    if ($udpTime+$udpPulse <=  $ev_daemontime) {
        ...
        &printEvent("HLSTATSX", "[UDP] No data since $_s seconds");
        $udpPulse += 120;
    }
    if ($httpTime+$httpPulse <=  $ev_daemontime) {
        my $_s = $ev_daemontime-$httpTime;
        &printEvent("HLSTATSX", "[HTTP] No data since $_s seconds");
        $httpPulse += 120;
    }
...
if ($g_stdin == 0) {
    ...
    if ($udp_socket) {
        ...
        $loop->reactor->io($udp_socket => sub {
            ...
            eval { handleIncoming("UDP", decode('utf8', $data)); };
            warn "[UDP] handleIncoming error: $@" if $@;
        });
    ...
    if ( $daemon ) {
        ...
        $daemon->on(request => sub {
            ...
            foreach my $line (@lines) {
                if ($line =~ /^(?:L\s*)?(\d{2}\/\d{2}\/\d{4} - \d{2}:\d{2}:\d{2}(?:\.\d{3})?)/) {
                    $line = "L $line" unless $line =~ /^L /;
                    push @processed, $line;
                }
            }
            ...
            for my $line (@processed) {
                eval { handleIncoming("HTTP", $line); };
                warn "[HTTP] handleIncoming error: $@" if $@;
            }
