...
sub rcon_getplayers_async {
    ...
    threads->create(sub {
        ...
        my $err;
        eval {
            $result = $self->rcon_getplayers();
        };
        $err = $@ if $@;
        $queue->enqueue([$err, $result]);
    })->detach;

    Mojo::IOLoop->recurring(0.1 => sub {
        return unless $queue->pending;
        my ($err, $res) = @{ $queue->dequeue };
        $callback->($err, $res) if $callback;
    });
}

sub track_server_load_async {
    my ($self, $callback) = @_;
    my $queue = Thread::Queue->new;
    ...
    threads->create(sub {
        ...
        $err = $@ if $@;
        $queue->enqueue([$err, $data]);
    })->detach;

    Mojo::IOLoop->recurring(0.1 => sub {
        return unless $queue->pending;
        my ($err, $data) = @{ $queue->dequeue };
...
sub get_map_async {
    ...
    threads->create(sub {
        ...
        my $err;
        eval {
            $result = $self->get_map($fromupdate // 0);  # call the blocking version
        };
        $err = $@ if $@;
        $queue->enqueue([$err, $result]);
    })->detach;
...
sub update_players_pings
{
    ...
    else
    {
        ...
        while ( my($pl, $player) = each(%{$self->{srv_players}}) )
        {
            ...
            {
                if ($player->{is_bot} == 0 && ( ($player->{userid} > 0) || (($player->{userid} >= 0) && ($self->{play_game} == CS2())) )   ) {
